...
def from_user_input():
    ...
    compatibility = (
        ask_input("Please enter the compatibility level [default is 1]: ") or "1"
    )
    module = Module(name, version, compatibility)

    url = ask_input("Please enter the URL of the source archive: ")
    strip_prefix = (
        ask_input("Please enter the strip_prefix value of the archive [default None]: ")
        or None
    )
    ...
    if yes_or_no("Do you want to add patch files?", False):
        ...
        patch_strip = (
            ask_input(
                "Please enter the patch strip number [Default is 1, compatible with git generated "
                "patches]: "
            )
            or "1"
        )
    ...
    presubmit_url = (
        "https://github.com/bazelbuild/bazel-central-registry/tree/main#presubmityml"
    )
    if yes_or_no(
        f"Do you want to specify an existing presubmit.yml file? (See {presubmit_url})",
        False,
    ):
        ...
        if yes_or_no("Do you have a test module in your source archive?", True):
            ...
            first = True
            while not (
                module.test_module_build_targets or module.test_module_test_targets
            ):
                if not first:
                    print(
                        "Build targets and test targets cannot both be empty, please re-enter!"
                    )
                first = False
                build_targets = ask_input(
...
def get_maintainers_from_input():
    ...
    while yes_or_no(
        f"Do you want to add {prefix} maintainer for this module?{explain}", False
    ):
        maintainer = {}
