...
""".strip()
    ...
    def get_metadata(self, module_name):
        metadata_path = self.root.joinpath("modules", module_name, "metadata.json")
        return json.load(metadata_path.open())

    def get_source(self, module_name, version):
        source_path = self.root.joinpath("modules", module_name, version, "source.json")
        return json.load(source_path.open())

    def get_source_path(self, module_name, version):
        return self.root.joinpath("modules", module_name, version, "source.json")

    def get_presubmit_yml_path(self, module_name, version):
        return self.root.joinpath("modules", module_name, version, "presubmit.yml")

    def get_patch_file_path(self, module_name, version, patch_name):
        return self.root.joinpath(
            "modules", module_name, version, "patches", patch_name
        )

    def get_module_dot_bazel_path(self, module_name, version):
        return self.root.joinpath("modules", module_name, version, "MODULE.bazel")
    ...
    def add(self, module, override=False):
        ...
        if self.contains(module.name, module.version):
            if override:
                log(
                    "Overridding module '%s' at version '%s'..."
